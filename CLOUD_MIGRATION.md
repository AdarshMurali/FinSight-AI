...
3. **Convert the Flink EC2's `/home/ec2-user/FinSight-AI/` to a proper git clone** (deploy key, sparse-checkout like the backend EC2) instead of the manual file-copy pattern — found 2026-07-14 that this box had silently drifted 5+ days stale, which is exactly the same class of bug as the original `risk_job.py` schedule outage, just from a different root cause (no git vs. wrong path). Low urgency since it was just fixed, but the underlying risk (any future change to `risk_job.py`/`alert_engine.py`/anything else it imports needs a remember-to-copy step) is still there until this is done.
