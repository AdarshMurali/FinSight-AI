...
6. **Market-hours-aware cost control.** The streaming EC2 (Kafka, Flink, Finnhub producers) is started and stopped daily by a scheduled Lambda, rather than running — and being paid for — 24/7.
...
| Streaming | Kafka (Confluent images) + Apache Flink 1.18 (PyFlink) | **Live** — separate EC2 `13.233.21.229` (t3.medium), market-hours-only (09:20–16:25 ET) |
...
- The streaming EC2 (t3.medium, market-hours-only) runs Zookeeper, Kafka, Kafka-UI, the Flink JobManager/TaskManager, and two Finnhub producer containers (trades + news). Flink jobs write embeddings back into ChromaDB on the *other* EC2 over a security-group-restricted cross-instance HTTP call.
- A scheduled **Lambda** (`finsight-ec2-scheduler`, EventBridge-triggered) starts and stops the streaming EC2 daily around market open/close. Separate EventBridge → SSM schedules run `risk_job.py` / `price_update_job.py` (streaming EC2) and the weekly `market_events_job.py` (backend EC2).
