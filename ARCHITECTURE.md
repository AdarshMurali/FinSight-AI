...
- **[`docs/architecture/functional-flow.svg`](docs/architecture/functional-flow.svg)** — the three dominant runtime pipelines (agentic AI chat, live volatility monitoring, and daily scheduled risk/data jobs), color-coded by deterministic-code vs. LLM vs. hybrid.
...
6. **Two EC2s, split by workload shape.** `finsight-flink` — Kafka, Flink, the Finnhub producers, and the two daily batch jobs — is kept on its own instance, separate from the backend/ChromaDB EC2, so a spike in streaming or batch load can't starve the API. It runs on a market-hours schedule (Lambda-managed EC2 power state + an on-box crontab for the pipeline itself), not 24/7 — see [§4](#4-system--deployment-architecture) for exactly how that scheduling works.
...
| Streaming | Kafka (Confluent images) + Apache Flink 1.18 (PyFlink) | **Live** — separate EC2 `13.233.21.229` (t3.medium), `finsight-flink`; market-hours-only (09:20–16:25 ET Mon–Fri), power-cycled by a scheduler Lambda with an on-box crontab starting/stopping the pipeline itself |
...
- The second EC2, `finsight-flink` (t3.medium), is dedicated to everything that needs to run continuously or on a schedule independent of a user's request — three distinct jobs:
  ...
  2. **Daily risk metrics** — `risk_job.py`, cron-triggered via EventBridge → SSM at 17:30 ET, recomputes VaR, stress-test, and factor-exposure numbers for every portfolio and writes `Risk_Metrics`, which the threshold-alert engine reads.
  3. **Daily end-of-day data capture** — `price_update_job.py`, EventBridge → SSM at 16:00 ET, pulls the day's OHLCV/dividends/splits/macro data (yfinance + FRED) so the next trading day starts with a current dataset in Azure SQL and ChromaDB. (An AWS Lambda version of this job was prototyped — `backend/lambda_price_update/` — but rejected: Lambda without a VPC has no static egress IP, and Azure SQL's firewall is IP-allowlisted, so every call was blocked. A NAT Gateway would fix that but costs more than just running the job on `finsight-flink`, which is already allowlisted. The SSM-on-EC2 path above is what's actually live.)

  Flink jobs write embeddings back into ChromaDB on the *other* EC2 over a security-group-restricted cross-instance HTTP call. The weekly `market_events_job.py` is the one scheduled job that runs on the *backend* EC2 instead, since it only needs REST/DB access, not the streaming stack.
- `finsight-flink` runs on a **two-layer market-hours schedule**, not 24/7: an AWS **Lambda** (`finsight-ec2-scheduler`), triggered by two EventBridge Scheduler rules (09:20 ET start, 16:25 ET stop, Mon–Fri, skipping NYSE holidays via a checked calendar), powers the EC2 instance itself on and off and logs each run to CloudWatch; a **crontab installed on the instance** separately starts and stops the Kafka/Flink Docker Compose pipeline within that window (`start_pipeline.sh` at 09:25 ET, `stop_pipeline.sh` at 16:15 ET) — instance power and pipeline lifecycle are deliberately separate concerns. The `risk_job.py` / `price_update_job.py` SSM schedules (above) are timed to land inside this window.
