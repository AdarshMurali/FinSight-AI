...
- **Schema** (`003_alerts_v2.sql`, `004_alerts_v3.sql`, additive, applied directly to production Azure SQL): `Alerts` gained `status` (active/resolved), `occurrence_count`, `last_triggered_at`, `resolved_at`, `read_at`.
...
1. **Data model** — `Users` table + `Portfolios.manager_id` FK, migrated via a new additive `backend/db/migrations/002_auth.sql` (NOT by re-running `backend/db/migrations/001_schema_and_seed.sql`, which deletes/recreates `Portfolios` and would have violated the FK from `Alerts`/`Risk_Metrics`, destroying the accumulated alert/risk history). 5 demo fund managers (~10 portfolios each) + 1 admin, documented in `AUTH.md`.
...
| **Live Market Events Feed** | ✅ Done | Found stale 2026-07-15, built and deployed 2026-07-16. One-time backfill (`backend/backfill_scripts/market_events_backfill.py`, not scheduled) populated Jan 1 – Jul 16 2026: 214 earnings (yfinance), 12 macro prints (FRED CPI + Payrolls), 0 FOMC decisions, 187 M&A/guidance-cut events (Finnhub, necessarily thin — free tier caps company-news lookback to ~2-3 days regardless of requested range; economic-calendar/upgrade-downgrade endpoints are 403 on this tier). Recurring `backend/scripts/market_events_job.py` runs weekly (Sundays 6:15 AM ET) via a new EventBridge schedule on the **backend EC2** (always-on, not the market-hours-only Flink EC2 — required first-time SSM setup there, see status.md). Auto-derives `affected_sectors`/`affected_regions` from the tickers involved, fixing the exposure-detection gap from 2026-07-15. Found and fixed 5 real data-quality bugs along the way (earnings quarter mislabeling, roundup-article misattribution, multi-ticker duplicate rows, same-story-multiple-outlets duplicate rows, insider-transaction misclassification) — full writeup in status.md. Net result: 428 rows backfilled, cleaned down to 316 after dedup, verified live on production sorted newest-first. |
...
### Task 7.3: Volatility Detector Overhaul + RAG Cross-Ticker Fix + DB-Driven Coverage — ✅ COMPLETE (2026-07-18)

Four separate but related fixes, all found/built in the same session, all pushed to `adarsh` and progressing through CI/CD approval as of this writing:

**1. RAG retrieval bug — `search_market_context` surfaced unrelated tickers.** `MarketRAGEngine.retrieve_context()` (`backend/rag/query_engine.py`) did pure cross-collection semantic search with no ticker filter — a generic query like "why did AAPL see unusual trading activity" could surface `volatility_events` docs for AXP/ADBE instead, since those docs' templated sentence structure embedded close to the query and won on the recency blend, with no actual AAPL doc competing in that collection's top-3. Fixed by adding an optional `ticker` param to `retrieve_context()`/`build_context_prompt()`, applied as a hard `where={"ticker": ...}` filter before ranking. `ai_tools.py`'s `search_market_context` tool schema now exposes this to GPT-4o (which already extracts entities like "AAPL" for other tools). Verified live against production ChromaDB: AXP/ADBE noise gone, real AAPL volatility docs surfaced instead. Files: `backend/rag/query_engine.py`, `backend/services/ai_tools.py`.

**2. Volatility detector thresholds + window size made SSM-configurable.** `backend/flink/volatility_detector_job.py` had `PRICE_MOVE_THRESHOLD_PCT`/`VOLUME_SPIKE_MULTIPLIER`/`WINDOW_MINUTES` hardcoded — tuning any of them meant a code change + redeploy. Now:
- The two per-window thresholds re-poll SSM every 5 min (`SSM_REFRESH_SECONDS`) while the job runs — no resubmit needed to change them.
- `WINDOW_MINUTES` is different — it's baked into the Flink job graph (`TumblingProcessingTimeWindows`) at submission time, so it's read once from SSM at module-import time, not polled; changing it needs a job resubmit. Live testing showed the original 5-minute window essentially never fired on price (1.5% inside 5 min is rare for the covered large-caps) — default raised to **15 minutes**.
- New SSM params: `/finsight/volatility_price_threshold_pct` (1.5), `/finsight/volatility_volume_spike_multiplier` (2.0), `/finsight/volatility_window_minutes` (15). IAM: new inline policy `finsight-volatility-ssm-read` on the Flink EC2's role (`finsight-ec2-instance-role`), `ssm:GetParameter` scoped to just these params.

**3. A second, independent "daily" volatility check added.** The existing per-window check only ever caught a spike *within* one window — a stock bleeding down 5% gradually across a full day, with no single window ever crossing threshold, was invisible. Added cumulative day-scoped tracking (`day_open_price`, `day_cum_volume`, `day_windows_seen` per symbol, reset on trading-date rollover) alongside the existing per-window state. Two new SSM-backed thresholds (`/finsight/volatility_daily_price_threshold_pct`=5.0, `/finsight/volatility_daily_volume_multiplier`=3.0), same refresh/fallback behavior as the window ones. Writes a distinct `scope="daily"` doc to `volatility_events` (vs. `scope="window"` on the existing doc type), at most once per symbol per day via a dedupe set — mirrors the dedup philosophy already established in `alert_engine.py`'s threshold-alert family upsert.

**4. Flink EC2 brought onto AWS Secrets Manager/SSM — was the one box still on plaintext `.env`.** Task 7.2 (above) only ever covered the backend/ChromaDB EC2. The Flink EC2 read `OPENAI_API_KEY`/`FINNHUB_API_KEY`/`CHROMA_HOST` straight from a plaintext `.env` file the whole time. New `backend/flink/secrets_loader.py` (identical shape to the backend's version, reuses the *same* `finsight/prod` secret and `/finsight/chroma_host` SSM param — no duplicated values), wired into `config.py` (covers both Finnhub producers since they import it), `volatility_detector_job.py`, and `news_sentiment_job.py` (neither imports `config.py`, so each needed its own call). Also found and fixed a real bug while checking this: `backend/mcp_server.py` had `USE_AWS_SECRETS=true` set in its container env but **never called the loader** — `FINSIGHT_MCP_TOKEN` was silently only ever read from a plain env var this whole time. New IAM policy `finsight-secrets-read` on the Flink EC2's role (`secretsmanager:GetSecretValue` on `finsight/prod`, `ssm:GetParameter` on `/finsight/chroma_host`).

**5. Ticker universe made DB-driven instead of a second hardcoded list.** Follow-up question during this same work: "is the volatility detector's coverage hardcoded?" — yes, `config.py`'s `TICKERS` was a static 50-symbol list, and only 39/135 securities actually held across portfolios were covered (96 held securities, including `GS`, `BAC`, `SPY`, `QQQ`, `TLT`, `TSM`, got zero real-time coverage). Added `GET /internal/tickers` (new `backend/routers/internal.py`, unauthenticated — ticker symbols aren't sensitive, and a service JWT for the Flink box's machine-to-machine call would just add a token-refresh problem for no security benefit), filtered to `security_type IN ('stock', 'etf')` (excludes commodity/currency rows like `GC=F`/`EURUSD=X`, not tradeable via Finnhub's equity websocket the same way) — returns 125 tickers today. `config.py` fetches this once at startup, falls back to the old hardcoded 50 if the backend's unreachable. New tickers added to `Securities` are picked up automatically next time the Flink EC2 restarts (already daily, around market hours) — no code change needed. Also fixed `Dockerfile.producer`, which only `COPY`'d 3 files and was missing the new `secrets_loader.py` from item 4 above — would have crashed both producers with an `ImportError` on next deploy.

**Verified end-to-end locally before pushing**: started the backend, hit `/internal/tickers` directly (confirmed 125 stock/ETF tickers), confirmed `config.py` picks up that exact list when pointed at the running server and falls back cleanly to the hardcoded 50 when the backend's unreachable. RAG ticker filter verified live against production ChromaDB (before/after comparison, cross-ticker noise eliminated). SSM threshold plumbing verified via direct `aws ssm get-parameters` + IAM policy `get-role-policy` checks.

**Deployed 2026-07-18, with one real incident along the way (found, fixed, and resolved same day):**

After all 4 feature commits (`a353384`, `130d007`, `32296d4`, `5a07011`) landed via CI/CD, both `deploy-backend` and `deploy-flink` reported `Success` — but a post-deploy check ("is everything actually in place") found the `trade-producer`/`news-producer` containers crash-looping (`Restarting (1)`), not just deployed-and-idle. Root cause, from the actual traceback:
```
...
```
`boto3` was added to the main Flink `Dockerfile` (jobmanager/taskmanager) in commit `32296d4`, but never to `Dockerfile.producer` — which builds a separate, smaller image for the two Finnhub producers — even though that same commit turned on `USE_AWS_SECRETS=true` for the producers too. Every container restart hit the same `ModuleNotFoundError` at import time before the process ever got past `config.py`. **Net effect: no real-time trades/news flowed into Kafka for roughly 24–30 minutes** (the gap between the `32296d4` deploy landing and the fix below).

Fixed by adding `boto3>=1.34.0` to `Dockerfile.producer`'s pip install list (commit `153825f`), pushed, CD approved and re-deployed. Verified via the actual production trade-producer log post-fix:
```
...
```
That single log line confirms three things at once, all working end-to-end in production: secrets loading fixed, the crash resolved, and the DB-driven ticker list (item 5 above) actually live — 125 symbols, not the old hardcoded 50.

**The originally-predicted "known gap" (ticker-list change needing a manual job resubmit) turned out to be moot** — `volatility_detector_job.py` doesn't reference `TICKERS` at all (only the trade producer does, to know what to subscribe to on Finnhub's websocket), so there was nothing for that job to "miss" by not being resubmitted.

**Not independently re-verified yet**: whether a fresh `volatility_events` doc actually shows `window_minutes: 15`/`scope: "window"|"daily"` in practice — checked the collection post-fix and the newest document was from 2026-07-17 14:40 UTC (today, 2026-07-18, is a Sunday — effectively no real trading activity to generate a fresh event regardless of code correctness). Re-check once markets are open (next weekday).

**Deliberately left as-is, not urgent**: `backend/rag/analyst_research_loader.py` and `backend/rag/load_financial_data.py` still have their own hardcoded ~50-ticker defaults — both are manual one-off CLI backfill scripts that already accept a `--tickers` override, not part of any live/continuous path. `FINSIGHT_MCP_TOKEN` was checked and confirmed already present in the `finsight/prod` secret (no merge needed — a fresh token was minted for `admin@finsight.demo` during this session in case it was needed, turned out not to be). `NGROK_API_KEY` confirmed dead (grepped, zero `.py` references) — recommended removing from local `.env`, not done yet.
...
**Status**: All of Phase 5 (5.1, 5.2 incl. event + AI alerts, 5.3, 5.4 PDF reports) complete. 3.4b (FastMCP), 6.2 (Redis/Valkey caching), 6.4 (JWT + multi-tenant access), 7.2 (AWS Secrets Manager + SSM Parameter Store), 7.3 (volatility detector overhaul + RAG ticker fix + DB-driven coverage) also complete and deployed to production 2026-07-18, including a same-day incident (producer crash loop, ~24-30 min, found post-deploy and fixed — see Task 7.3 above). AWS ChromaDB 36K+ docs. Docker Desktop retired. Both daily EC2 batch jobs (`risk_job.py`, `price_update_job.py`) live and verified on AWS (2026-07-08). FastAPI backend pushed to the cloud 2026-07-09 (Task 6.3 partial — see `CLOUD_MIGRATION.md`): reused ChromaDB EC2, `fin-sightai.space` domain, nginx+HTTPS, Vercel frontend live at `https://www.fin-sightai.space`. MCP server relocated from the Flink EC2 to the ChromaDB EC2 2026-07-11 (that box resized t3.micro → t3.small to fit it, now also running Redis/Valkey). Overview tab volatility display bug fixed 2026-07-11. Stress testing overhauled 2026-07-12: swapped Dot-com Bust for 2023 Regional Banking Crisis + 2026 Iran War, added 4 parametric shock scenarios (Rates ±100bps, Equities -20%, No Stress) as a separate sensitivity-based methodology — see status.md for full writeup. Dockerization Phase 1 (backend EC2) done 2026-07-16; Phase 2 (Flink EC2) 2026-07-16. Orphaned analyze-event AI endpoint (found 2026-07-12) got a UI 2026-07-15/16. Real CI/CD done 2026-07-17. Volatility detector overhaul + Flink onto AWS Secrets Manager + DB-driven ticker coverage deployed 2026-07-18 (see Task 7.3 above) — one incident along the way (producer crash loop from a missing `boto3` dependency, found via a post-deploy check, fixed same day, ~24-30 min of no real-time trade/news ingestion). Pending: Excel report format (deferred), analyst_research_loader.py/load_financial_data.py ticker lists still hardcoded (low priority, manual scripts), re-verify window_minutes=15/scope field on a live volatility_events doc once markets are open (checked immediately post-fix but it was a Sunday, no fresh trading activity to confirm against).
