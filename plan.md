...
**Not yet deployed as of this writing** — 3 commits pushed to `adarsh` (`130d007`, `32296d4`, `5a07011`, plus the RAG fix in `a353384`), CI will rebuild the `backend`/`mcp` and `producer` images (not `flink`, since the main Flink Dockerfile is untouched this round), both `deploy-backend`/`deploy-flink` CD jobs need manual approval. **One known gap**: `volatility_detector_job.py` itself didn't change in the ticker-list commit (only `config.py`, which it imports) — `start_pipeline.sh`'s hash-based redeploy tracks specific job entrypoint files, not their shared imports, so the already-running job may need a manual cancel/resubmit to actually pick up the dynamic ticker fetch, even after `deploy-flink` lands.
