...
| **Live Market Events Feed** | High | Found 2026-07-15. `Market_Events` (SQL table) has no live ingestion — no scheduled job or stream keeps it current, so the market-events pages and the AI event analyzer (UI added 2026-07-15) are working against stale/incomplete event data. Plan (2026-07-16, not started): **(1) Cadence** — daily batch job, not a live stream; this feature is retrospective/explanatory (computes exposure + narrates impact on events that already happened), not predictive, so hours-to-a-day-old is fine — reuse the existing `risk_job.py`/EventBridge scheduled-job pattern rather than building new streaming infra. **(2) Source** — Finnhub (already integrated, key already in `.env`): earnings calendar + company/general news endpoints. **(3) Scope** — curated/structured events only (earnings, FOMC/Fed decisions, major M&A, guidance cuts/downgrades, macro prints); the existing live `market_news`/`volatility_events` ChromaDB pipeline already covers the high-volume news firehose, `Market_Events` should stay selective. **(4) Critical** — auto-derive `affected_sectors`/`affected_regions` from the tickers involved in each event at ingest time; untagged events silently break exposure detection in `event_analyzer.py` (root cause of the 2026-07-15 "0 affected portfolios" bug on an untagged earnings event). **(5) Bonus/cheap win** — auto-promote high-severity hits from the already-running `volatility_detector_job` (Flink) into `Market_Events`, since those already carry tickers to derive tags from, no new data source needed. |
