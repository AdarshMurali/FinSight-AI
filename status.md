...
- ✅ Parametric shock scenarios — COMPLETE, deployed (2026-07-12)
  ...
  - New `Risk_Metrics.parametric_data` column (`backend/db/migrations/005_parametric_shocks.sql`, additive/idempotent, applied to production Azure SQL) — kept separate from `stress_data` on purpose, since mixing historical-replay and sensitivity-based numbers in one JSON blob would blur two different kinds of evidence.
...
- ✅ Task 6.4: JWT Auth + Multi-Tenant Portfolio Access — COMPLETE, deployed to production (2026-07-11)
  - **Data model**: `Users` table + `Portfolios.manager_id` FK, via a new additive `backend/db/migrations/002_auth.sql` (not by re-running `backend/db/migrations/001_schema_and_seed.sql` — that deletes/recreates `Portfolios`, which would violate the FK from `Alerts`/`Risk_Metrics` and wipe the accumulated alert/risk history built up in Tasks 5.1/5.2). 5 demo managers (~10 portfolios each) + 1 admin — see `AUTH.md` for credentials.
...
- ✅ Alert dedup, mute cooldown, portfolio-name display — COMPLETE, deployed to production (2026-07-15)
  ...
  - **Schema** (`backend/db/migrations/003_alerts_v2.sql`, `004_alerts_v3.sql` — additive, applied directly to production Azure SQL): `Alerts` gained `status` (active/resolved), `occurrence_count`, `last_triggered_at`, `resolved_at`, `read_at`.
...
| 6 — Infrastructure | ✅ Complete | Backend cloud deploy ✅ · 6.2 Redis/Valkey caching ✅ · 6.4 JWT auth ✅ · 6.1 Dockerization ✅ (both EC2s, 2026-07-16) · 6.3 real CI/CD ✅ (both environments verified end-to-end, 2026-07-17) |
...
- ✅ Task 6.3: Real CI/CD — done (2026-07-17). GitHub Actions for both CI (build/push Docker images to the user's Docker Hub) and CD (deploy via SSM, gated behind separate manual-approval GitHub Environments per EC2 — `production-backend`/`production-flink` — so a backend-only change can be approved without also redeploying Flink EC2, and vice versa). AWS auth via OIDC federation (no long-lived AWS keys stored in GitHub). `backend/backfill_scripts/**` deliberately excluded from the whole pipeline — one-time scripts, never auto-deployed.
  - **CI workflow live**: `.github/workflows/ci.yml` — path-filtered (`dorny/paths-filter`) so a backend-only change doesn't rebuild the Flink/producer images and vice versa. Builds `finsight-backend`, `finsight-flink`, `finsight-producer`; PRs build without pushing (verifies the Dockerfile still works without touching `latest`).
  - **Real bug caught on the very first run**: the path filter used `'backend/**'` + `'!backend/flink/**'` negation patterns — `dorny/paths-filter`'s default "some" quantifier doesn't combine positive/negative patterns the way it looks like it should, and a push touching *only* `.github/workflows/ci.yml` incorrectly matched the "backend" filter (confirmed via the action's own diff log), triggering a real build that then failed at Docker Hub login (no credentials configured yet, expected). Fixed by switching to positive-only patterns enumerating `backend/`'s actual subdirectories — re-verified with a real run showing both build jobs correctly `skipped` for a docs-only change.
  - **Also caught before it could break CI**: `backend/flink/Dockerfile`'s `flink-sql-connector-kafka-3.1.0-1.18.jar` dependency is gitignored (large binary, kept off both EC2s' git checkouts on purpose) — a fresh CI checkout wouldn't have it. Added a Maven Central fetch step to the Flink build job (verified the URL is reachable and returns the exact same byte size, 5,596,853 bytes, as what's deployed on the EC2, before relying on it).
  - **Known gap, not silently ignored**: `test_api.py`/`test_phase3.py` can't run in CI as-is — both need a live server + live Azure SQL connection (IP-allowlisted, unreachable from GitHub-hosted runners), `test_phase3.py` also makes real OpenAI calls. Making them CI-safe needs real rework (mocking the DB/API layers) — tracked as a follow-up. CI currently runs a Python syntax/import check as the practical stand-in.
  - **Also caught while designing the CD path-filter logic, unrelated to CI itself**: asking "would a backend-only change also redeploy Flink EC2" surfaced that yesterday's Flink EC2 git-conversion sparse-checkout was scoped too narrowly and had already broken `risk_job.py`/`price_update_job.py` — see the dedicated entry above (2026-07-17) for the full incident and fix, plus the manual catch-up run confirming both jobs work again (50/50 portfolios succeeded).
  - **Docker Hub + AWS OIDC set up, CD workflow built**: `DOCKERHUB_USERNAME`/`DOCKERHUB_TOKEN` set on the repo; CI confirmed actually pushing images to `adarshmurali/finsight-backend`/`finsight-flink`/`finsight-producer` on Docker Hub. AWS OIDC federation set up (`aws/iam/README.md`) — IAM role `finsight-github-actions-cd`, trust policy scoped to this repo's environment-targeting jobs only, no long-lived AWS keys in GitHub. `.github/workflows/cd.yml` — two independent jobs (`deploy-backend`/`deploy-flink`), each gated behind its own GitHub Environment approval (`production-backend`/`production-flink`), triggered by `workflow_run` after CI succeeds. Flink EC2 job starts the box first if it's stopped (only inside the already-approved job, never as an unconditional daily action — an earlier "self-sync daily pull" design was rejected as breaking the approval gate) and stops it again only if it's what started it.
  - **Three real bugs found and fixed across the first three `production-backend` runs (2026-07-17)**, layered on top of each other — each fix uncovered the next:
    1. `ssm:GetCommandInvocation` doesn't support resource-level IAM scoping at all — AWS requires `Resource: "*"` for it (confirmed via AWS's own docs). The original policy scoped it to the two instance ARNs like `ssm:SendCommand`, which `AccessDeniedException`'d immediately after `send-command` had already fired. Fixed by splitting it into its own statement with `Resource: "*"` (`aws/iam/github-actions-cd-permissions.json`, re-applied live via `aws iam put-role-policy`).
    2. Underneath that: `git pull` had been silently failing on every SSM-driven deploy (`fatal: detected dubious ownership in repository` — SSM runs as root, the checkout is owned by `ec2-user`). A `git config --global --add safe.directory` fix looked right but didn't actually apply — SSM's shell has no `$HOME` set, so `--global` itself fails (`fatal: $HOME not set`), silently swallowed by the script's own `||` fallback. Worse, `AWS-RunShellScript` has no implicit exit-on-error and its `Status` only reflects the *last* line's exit code, so `docker compose up -d` succeeding against stale local state made the whole run report `Success` regardless — a `set -e` was missing the whole time.
    3. Root cause, finally: the deploy key, `~/.ssh/config` (`IdentityFile`, `StrictHostKeyChecking accept-new`), and `known_hosts` all live only under `ec2-user`'s home — never copied to root's. `--system` git config (writing `/etc/gitconfig`, no `$HOME` dependency) fixed the ownership check, but root still couldn't SSH to GitHub at all (`Host key verification failed`). Real fix: run the pull itself as `sudo -u ec2-user -H git pull`, sidestepping both the ownership and SSH-credential problems in one step, rather than trying to replicate ec2-user's SSH setup under root. Confirmed live: this was the *first successful automated `git pull`* on this box ever — pulled 17 real files (including the CI/CD workflow and IAM setup files themselves) that no automated path had ever landed before.
  - **Verified for real, not just "job passed green"**: after the third fix, independently SSM'd into the box and confirmed `git log -1` matches the exact pushed commit, `docker-compose.yml` shows the correct `adarshmurali/finsight-backend:latest` image, both `finsight-backend`/`finsight-mcp` containers were recreated seconds earlier on that image, and `https://api.fin-sightai.space/health` returns `{"status":"healthy","database":"connected"}`.
  - **`production-flink` approved and tested (2026-07-17)**, confirming the two Environments gate independently as designed (`production-backend` stayed un-approved on the same run throughout). Two more real, separate bugs found:
    4. `aws ssm wait command-executed`'s default max wait (~100s) isn't enough for a cold `docker compose pull` of the Flink image (PyFlink + chromadb + openai deps) — the first `production-flink` attempt gave up and reported failure while the underlying SSM command was still healthy and `InProgress`, then stopped the EC2 anyway (the cleanup step runs regardless of the deploy step's outcome). Fixed by replacing the built-in waiter with a manual poll loop in both jobs (60×15s backend, 90×15s Flink — `.github/workflows/cd.yml`).
    5. Deeper, pre-existing bug unrelated to CD itself: `start_pipeline.sh` submits both Flink jobs (`flink run`) unconditionally on every invocation, with no check for an already-running instance. A retried deploy created a second concurrent pair of the same two jobs; with only 4 total task slots, the two pairs fought over them and **all four jobs ended up in a terminal `FAILED` state within ~20 minutes** (`NoResourceAvailableException: Could not acquire the minimum required resources`, no restart strategy configured) — not a code bug in the jobs themselves, pure resource contention. Fixed by adding a `job_is_running()` check (`aws/ec2-flink/start_pipeline.sh`) that queries Flink's `jobs/overview` REST API before submitting each job, skipping if a `RUNNING` instance with that name already exists. The JSON-parsing approach (nested `"tasks":{...}` breaks naive `grep -oE '\{[^{}]*\}'` brace-matching — first attempt at this silently matched each job's *inner* tasks object and would never have found `"name"` at all) was verified against both a synthetic payload and the box's real live API output before trusting it, given the first version would have silently never fired.
  - **Cleanup + real end-to-end verification**: canceled the 2 stale duplicate jobs, resubmitted one clean healthy pair (confirmed `RUNNING` with all task slots allocated, 0 failed), pushed the idempotency fix, pulled it onto the box (`sudo -u ec2-user -H git pull`, confirmed HEAD matches), then re-ran `start_pipeline.sh` again live and confirmed it now logs "already RUNNING, skipping submission" for both jobs instead of creating more duplicates — before stopping the EC2 again to restore its normal off-hours state ahead of the existing cron/Lambda market-hours schedule.
  - **Next**: nothing outstanding on the CD pipeline itself — both environments verified independently end-to-end with real bugs found and fixed at every layer (IAM scoping, git ownership/SSH credentials, SSM wait timeouts, Flink job idempotency). Possible future cleanup: the `start_pipeline.sh` "Verifying state" log loop (unchanged) still prints every job matching a name, including old terminal-state history — cosmetic only, not correctness-affecting.
  - **Verified the git-pull deploy path for non-containerized scheduled scripts (2026-07-17)**, and corrected a wrong assumption while doing it: `risk_job.py` and `price_update_job.py` actually run on the **Flink EC2** (`i-06df445415d082798`, via `mcpvenv`), not the backend EC2 — confirmed directly from both EventBridge Scheduler targets (`aws scheduler get-schedule --name finsight-daily-risk-job`/`finsight-daily-price-update`, both target `i-06df445415d082798`). Pushed a temporary marker log line into `risk_job.py`, deployed through `production-backend` first (wrong box, harmless), then through `production-flink` (right box), confirmed the file changed on the Flink EC2's checkout, then **actually executed `risk_job.py` live** (`mcpvenv/bin/python scripts/risk_job.py`) and saw the marker in real runtime output alongside genuine work (DB connect, 50 portfolios, live ChromaDB/OpenAI calls) — not just a file-presence check. Reverted the marker and redeployed the cleanup to both boxes, confirmed gone on both.
  - **Correction, found later the same session**: `market_events_job.py` does *not* share the Flink EC2 with `risk_job.py`/`price_update_job.py` as originally assumed above — checked its actual EventBridge Scheduler target (`finsight-weekly-market-events-job`) and it runs on the **backend/ChromaDB EC2** (`i-0d5332841d8f8da41`, via `backendvenv`), covered by `production-backend`'s `git pull` instead. Not separately executed with its own live marker test, but the same mechanism is proven by the backend EC2's other verified deploys.
  - **Full CI/CD coverage audit (2026-07-17)**: went through every job/service against CI/CD explicitly (backend, MCP, Flink jobmanager/taskmanager, producers, both Flink streaming jobs, risk/price/market-events scripts, alerts) — all covered and either directly tested or covered by an already-verified identical mechanism. Two genuine gaps found, both **confirmed as deliberate manual-deploy exceptions per user decision (2026-07-17)**, not planned for automation: **ChromaDB** (separate compose project, excluded by original Dockerization design) and **`aws/lambda/ec2_scheduler.py`** (the market-open/close EC2 power-scheduler Lambda — source is version-controlled but not referenced by `ci.yml`/`cd.yml`, needs manual `aws lambda update-function-code`; new finding, not a regression).
  - **Hash-based redeploy fix implemented and verified live (2026-07-17)**, closing the gap above. `deploy_job()` in `start_pipeline.sh` now tracks a `sha256sum` of each job's `.py` file under `/home/ec2-user/.flink_job_versions/<job>.sha256` (world-writable — `chmod 777`/`666` — since this script runs as root via CD/SSM but as ec2-user via cron, the same ownership split that broke git pull and SSH earlier today would otherwise let whichever user writes first lock the other out). Same name + same hash = skip (unchanged). Same name + different hash = cancel then resubmit. No running job = submit fresh either way. Verified against all 4 scenarios first with mocked docker/Flink calls locally, then for real: a fresh first deploy (both jobs submitted, hashes recorded and matched sha256sum independently), a true no-op redeploy (identical JIDs, skipped), and a real code change (old JID `8b062d69` → `CANCELED`, new JID `a61359a8` → `RUNNING`, while the untouched job kept its exact JID `593123711acb` throughout).
  - **Second real bug found while verifying the above, fixed in the same pass**: the first code-change test looked right (both jobs got new JIDs) but for the wrong reason — `.github/workflows/ci.yml`'s single combined `flink: backend/flink/**` filter rebuilt the whole `finsight-flink` image for a comment-only change to a job `.py` file, even though that image's `Dockerfile` doesn't `COPY` any job files at all (the Flink jobs get their code purely via the docker-compose volume mount). The rebuilt image's new digest made `docker compose up -d` recreate the *jobmanager container itself* on the next deploy, wiping every running job — masking whether the hash logic's surgical single-job behavior actually worked. Fixed by splitting the filter into `flink_image` (`Dockerfile` only) and `producer_image` (`Dockerfile.producer` + its actual `COPY` targets: `config.py`, both producer scripts) — `finsight-producer` genuinely bakes those files in (no volume mount override), so it correctly still rebuilds on changes to them. Re-verified end to end after the split: a job-file-only change now shows `build-flink: skipped` in CI, and the resulting deploy left the jobmanager container's `docker inspect ... StartedAt` timestamp completely unchanged while still correctly cancelling+resubmitting only the one job whose code changed.

- ✅ Task 7.3: Volatility Detector Overhaul + RAG Cross-Ticker Fix + DB-Driven Coverage (2026-07-18). Four related fixes, full writeup in `plan.md` Task 7.3 — summarized here:
  1. **RAG bug found via user testing**: asked the chat "why did AAPL see unusual trading activity recently?" and got AXP/ADBE volatility events back instead of AAPL's own — `MarketRAGEngine.retrieve_context()` did pure cross-collection cosine similarity with no ticker filter, so a same-shaped `volatility_events` doc for an unrelated ticker could out-rank the real one on recency alone. Fixed with an optional `ticker` param (`query_engine.py`) applied as a hard `where` filter, exposed to GPT-4o via the `search_market_context` tool schema (`ai_tools.py`). Verified live against production ChromaDB: AAPL query now surfaces AAPL's own volatility docs, not AXP/ADBE.
  2. **Volatility detector thresholds made SSM-configurable** (`volatility_detector_job.py`) — `PRICE_MOVE_THRESHOLD_PCT`/`VOLUME_SPIKE_MULTIPLIER` re-poll SSM every 5 min while running, no redeploy needed to tune. `WINDOW_MINUTES` is structurally different (baked into the Flink job graph at submission, can't be hot-reloaded) — read once from SSM at import time instead; live testing showed 5 minutes essentially never fired on price, raised default to **15**. New SSM params + a new `finsight-volatility-ssm-read` IAM policy on the Flink EC2's role.
  3. **New "daily" spike check added** — the existing per-window check missed a slow bleed that never crosses threshold in any single window. Added cumulative day-scoped tracking (reset on trading-date rollover), two new SSM thresholds (default 5%/3x), writes a distinct `scope="daily"` doc (vs. `scope="window"`), deduped to at most one per symbol per day.
  4. **Flink EC2 brought onto AWS Secrets Manager/SSM** — Task 7.2 only ever covered the backend EC2; Flink was still reading `OPENAI_API_KEY`/`FINNHUB_API_KEY`/`CHROMA_HOST` from a plaintext `.env`. New `backend/flink/secrets_loader.py` (same shape as the backend's, reuses the *same* secret/param, no duplication), wired into `config.py`/`volatility_detector_job.py`/`news_sentiment_job.py`. **Real bug found while checking this**: `mcp_server.py` had `USE_AWS_SECRETS=true` set but never actually called the loader — `FINSIGHT_MCP_TOKEN` was silently only ever a plain env var this whole time. New `finsight-secrets-read` IAM policy on the Flink EC2's role.
  5. **Ticker coverage made DB-driven** — follow-up question surfaced that `config.py`'s `TICKERS` was a hardcoded 50-symbol list covering only 39 of the 135 securities actually held across portfolios (96 held securities — `GS`, `BAC`, `SPY`, `QQQ`, `TLT`, `TSM`, etc. — got zero real-time volatility coverage). New `GET /internal/tickers` (`backend/routers/internal.py`, deliberately unauthenticated — non-sensitive data, avoids a service-JWT refresh problem for the Flink box), filtered to `security_type IN ('stock','etf')`, returns 125 tickers today. `config.py` fetches this at startup, falls back to the old hardcoded 50 if unreachable. New tickers in `Securities` get picked up automatically on the Flink EC2's next daily restart — no redeploy needed. **Also fixed**: `Dockerfile.producer` was missing `secrets_loader.py` in its `COPY` list (introduced by fix #4 above) — would have crashed both producers with an `ImportError` on next deploy.
  - **Verified end-to-end before pushing**: started the backend locally, hit `/internal/tickers` directly (125 tickers), confirmed `config.py` picks up that list and falls back cleanly when the backend's unreachable. RAG fix verified with a live before/after query against production ChromaDB.
  - **Files changed**: `backend/rag/query_engine.py`, `backend/services/ai_tools.py`, `backend/flink/volatility_detector_job.py`, `backend/flink/secrets_loader.py` (new), `backend/flink/config.py`, `backend/flink/news_sentiment_job.py`, `backend/flink/Dockerfile`, `backend/flink/Dockerfile.producer`, `backend/mcp_server.py`, `backend/main.py`, `backend/routers/internal.py` (new), `aws/ec2-flink/docker-compose.yml`.

  - ⚠️ **Incident, same day: producer crash loop, ~24-30 min of no real-time trade/news ingestion, found and fixed within the same session.** All 4 commits above deployed cleanly per CI/CD (`deploy-backend`/`deploy-flink` both reported `Success`) — but a deliberate post-deploy check ("is everything actually in place", not just "did the pipeline report success") found `trade-producer`/`news-producer` crash-looping (`docker ps` showing `Restarting (1)`). Actual traceback:
    ```
    File "/app/config.py", line 8, in <module>
        load_aws_secrets()
    File "/app/secrets_loader.py", line 27, in load_aws_secrets
        import boto3
    ModuleNotFoundError: No module named 'boto3'
    ```
    **Root cause**: `boto3` was added to the main Flink `Dockerfile` (jobmanager/taskmanager) but never to `Dockerfile.producer` — a separate, smaller image for the two Finnhub producers — even though the same commit (`32296d4`) turned on `USE_AWS_SECRETS=true` for the producers too. Every restart hit the same import-time crash before the process got anywhere near Kafka.
    **Fixed**: added `boto3>=1.34.0` to `Dockerfile.producer` (commit `153825f`), pushed, CD approved, re-deployed. **Verified via the real production trade-producer log post-fix**:
    ```
    [OK]   Loaded 12 secrets from Secrets Manager (finsight/prod) + 1 params from SSM Parameter Store
    Symbols: 125
    Subscribed to: AAPL, ABBV, ABNB, ABT, ADBE, AEP, AGG, AMD, AMGN, AMT...
    ```
    That one log line confirmed three things at once, live in production: the crash fixed, secrets loading working, and the DB-driven ticker list (item 5 above) actually in effect — 125 symbols subscribed, not the old hardcoded 50.
    **The originally-predicted "known gap" (ticker-list change needing a manual job resubmit) turned out to be moot** — `volatility_detector_job.py` doesn't reference `TICKERS` at all, only the trade producer does, so there was nothing for the already-running job to miss.
    **Not independently re-verified yet**: whether a fresh `volatility_events` doc actually shows `window_minutes: 15`/`scope` in practice — checked the collection right after the fix and the newest doc was from 2026-07-17 14:40 UTC; today (2026-07-18) is a Sunday, so there's essentially no real trading activity to generate a fresh event regardless of code correctness. Re-check next weekday.
  - **Deliberately left as-is**: `analyst_research_loader.py`/`load_financial_data.py` still default to a hardcoded ticker list (manual CLI scripts, already accept `--tickers` override, not urgent). `FINSIGHT_MCP_TOKEN` checked and confirmed already in `finsight/prod` (no action needed). `NGROK_API_KEY` confirmed dead code, not yet removed from local `.env`.

**Current Focus**: All of Phase 5 is now complete and deployed, including Task 5.4 (PDF report generation) as of 2026-07-11. The Overview tab volatility/percentage display bug found while building the report, and a Risk Analytics alert panel bug found during a later user review (panel not hiding once fully read), are both fixed and deployed (see dedicated entries above). Task 7.2 (AWS Secrets Manager + SSM Parameter Store) is also complete and deployed — the backend EC2 now pulls all real credentials from AWS via its IAM role, with the plaintext `.env` stripped down to non-secret config only. The ChromaDB EC2 runs four services together (ChromaDB, backend, MCP server, Valkey) after being resized t3.micro → t3.small to fit them; the MCP server was also relocated there from the market-hours-only Flink EC2, and it's now SSM-managed too (added 2026-07-16 for the market-events weekly job — see below). The orphaned `analyze-event` AI endpoint now has a UI (2026-07-15/16), and `Market_Events` now has a real backfill + recurring weekly ingestion job instead of static 2025 seed data (2026-07-16, see dedicated entry above). Dockerization done 2026-07-16 (both EC2s), real CI/CD done 2026-07-17 (see Task 6.3 above) — Phase 6 is now fully complete. The "retire the stale pre-auth MCP server on the Flink EC2" item this line used to point to was itself already stale by the time it was written here — `CLOUD_MIGRATION.md`'s Session Log shows that cleanup was done 2026-07-15. Task 7.3 (volatility detector overhaul + RAG ticker fix + DB-driven coverage, see dedicated entry above) deployed 2026-07-18, including a same-day incident (producer crash loop from a missing `boto3` dependency, found via a post-deploy check and fixed within the session — see the entry's incident note). Everything confirmed healthy post-fix except the widened 15-minute window/daily-scope doc, which couldn't be independently verified against a live event since 2026-07-18 is a Sunday with no real trading activity. Next: re-verify that on the next trading day, then monitoring/log aggregation (Task 6.3's remaining deliverables), or whatever the user prioritizes next.

- ✅ Task 6.1: Dockerization — Phase 2, Flink EC2 (2026-07-16). Kafka/Zookeeper/Flink jobmanager/taskmanager were already Dockerized (`aws/ec2-flink/docker-compose.yml`). Added `trade-producer`/`news-producer` as two more services in that same file, `restart: unless-stopped` — replacing the bare `nohup` host processes that let the trade producer sit dead for ~18hrs on 2026-07-14 after a crash. Confirmed live while planning this change: two duplicate copies each of both producers were running simultaneously (`start_pipeline.sh` never killed existing ones before relaunching) — a second real symptom of the same underlying gap, also fixed by moving to named containers.
  - **Also converted this box to git** (sparse-checkout `aws/ec2-flink` + `backend/flink`, matching the backend EC2's pattern) rather than deferring further, per user decision made while planning this. New read-only deploy key (`finsight-flink-ec2`, separate from the backend EC2's own key). Migration was careful, not a plain `git init` in place: cloned fresh into a temp dir, diffed against the live directory, copied over untracked box-specific files (`.env`, the `flink-sql-connector-kafka-3.1.0-1.18.jar` binary Docker needs but git excludes), then swapped directories (old one kept as `FinSight-AI.bak.20260716`, not deleted). Confirmed the swap was transparent to the already-running containers — their bind mounts resolve by host path, not inode, so `docker exec ... ls /opt/flink/jobs` and the running Flink jobs were unaffected.
  - **Two live-vs-git drift bugs found during the diff, both fixed**: `crontab.txt` and `stop_pipeline.sh`'s log path still referenced a stale `/home/ubuntu/...`/`/var/log/...` convention from before this box was renamed to `ec2-user` — `start_pipeline.sh` had been fixed live at some point but never committed back (exactly the class of bug this whole git-conversion effort exists to prevent), the other two had silently been working around it via cron's own log redirect this whole time.
  - **Code change**: `backend/flink/config.py` already had `KAFKA_BOOTSTRAP_SERVERS_INTERNAL = "kafka:29092"` defined for exactly this case but unused — added a `KAFKA_MODE=docker` env check to pick it over the external IP:9092 route, set in the two new compose services. Producers now reach Kafka the same way the Flink jobs already do.
  - **`start_pipeline.sh` simplified, not just updated**: since `docker compose up -d` (no service filter, already Step 1) now starts the producers along with everything else, the old nohup+PID-file launch block was removed entirely rather than replaced. `stop_pipeline.sh`'s PID-file kill logic became `docker compose stop trade-producer news-producer`.
  - **Side benefit**: `flinkvenv` (the venv the old native processes used) is confirmed Python 3.9.25 — the exact version whose lack of PEP 604 union-type support caused the 2026-07-14 crash in the first place. Building the producer image on Python 3.11 removes the host-Python constraint entirely, so this exact bug class can't recur even if the `Optional[...]` fix were ever accidentally reverted again.
  - **Verified end-to-end**: cut over one producer at a time (trade first, then news), confirmed each connects via `docker logs`, confirmed `market.trades` topic offset climbing in real time (+195 in 8s), confirmed the Volatility Detector Flink job's source vertex `write-records` climbing continuously post-cutover (250542 → 251021 in 10s) and the downstream sink matching, confirmed fresh sentiment output flowing to ChromaDB in the taskmanager logs (News Sentiment Stream job). `ps aux` confirmed no leftover native producer processes; `docker ps` showed all 7 containers (`zookeeper`, `kafka`, `kafka-ui`, `flink-jobmanager`, `flink-taskmanager`, `trade-producer`, `news-producer`) `Up`.
  - **Both phases of Task 6.1 (Dockerization) are now complete.**
  - ⚠️ **Regression found and fixed 2026-07-17, one day later**: the git-conversion sparse-checkout (`aws/ec2-flink` + `backend/flink` only) was scoped for the Dockerization task's own needs and didn't account for `backend/scripts/risk_job.py`/`price_update_job.py` — also invoked via SSM on this same box, but living outside both scoped paths. Caught by planning a CI/CD path-filter design with the user, who asked "would a backend-only change also redeploy Flink EC2 stuff" — prompted a live check that found `backend/scripts/` entirely missing from the box, which would have failed today's 16:00/16:10 ET scheduled runs. Deeper than the missing directory: `risk_job.py` transitively imports `backend/services/risk_analytics.py` → `alert_engine.py` → `portfolio_analyzer.py` and `rag/query_engine.py` — both `backend/services/` and `backend/rag/` were *also* missing, which would have failed as a Python import error even if the script file itself had been found. Also found `backend/.env`, `mcpvenv/`, and `flinkvenv/` (all untracked, needed to be manually carried over during yesterday's migration, only `aws/ec2-flink/.env` and the Flink connector `.jar` were copied at the time) were missing entirely from the new checkout, only present in the `.bak` directory.
    - **Fixed**: `git sparse-checkout add backend/scripts backend/services backend/rag`; copied `backend/.env`, `mcpvenv/`, `flinkvenv/` from `FinSight-AI.bak.20260716/` into the live checkout.
    - **Verified**: both scripts' actual import chains succeed cleanly against `mcpvenv` (not just "file exists" — ran the real `from services.risk_analytics import compute_all` etc. imports the scripts themselves use).
    - **How to apply**: before scoping a sparse-checkout (or narrowing one further) on either EC2, trace the *transitive* import closure of everything actually invoked via cron/SSM on that box, not just the files directly referenced in the command line — a script's own imports can reach into directories that look unrelated to the task at hand.
...
- ✅ Task 6.1: Dockerization — Phase 1, backend EC2 (2026-07-16). ChromaDB was already Dockerized and left untouched (already stable, reachable via `CHROMA_HOST`/`PORT` — SSM Parameter Store resolves to the box's own public IP:8001 — identically from containers as from native processes, so no networking change needed there). Backend, MCP, and Valkey moved from native `systemd` units to Docker: `backend/Dockerfile` (one shared image for both `backend`/`mcp` services — same codebase/deps, only the entrypoint differs), `backend/requirements-docker.txt` (trimmed, same exclusions as the native venvs), `aws/ec2-backend/docker-compose.yml`.
  ...
  - **Phase 2 (Flink EC2) done same day** — see the entry above.
