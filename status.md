...
- ✅ Alert dedup, mute cooldown, portfolio-name display — COMPLETE, deployed to production (2026-07-15)
  ...
  - **Known regression surfaced, not caused by this feature**: restarting `finsight-mcp.service` (needed to pick up the new `models.py`) revealed its `FINSIGHT_MCP_TOKEN` no longer validates (`401: Invalid session`) — pre-existing since around the 2026-07-11 AWS Secrets Manager `JWT_SECRET_KEY` migration, just never surfaced because the old process hadn't been restarted since. Fix requires minting a fresh admin-level token, held pending explicit user go-ahead (the safety layer correctly flagged this as outside the literal deploy/test scope). MCP server currently down; FastAPI backend and frontend are unaffected.
