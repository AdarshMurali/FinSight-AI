...
- 🔄 Task 6.3: Real CI/CD — in progress (2026-07-17). Plan: GitHub Actions for both CI (build/push Docker images to the user's Docker Hub) and CD (deploy via SSM, gated behind separate manual-approval GitHub Environments per EC2 — `production-backend`/`production-flink` — so a backend-only change can be approved without also redeploying Flink EC2, and vice versa). AWS auth via OIDC federation (no long-lived AWS keys stored in GitHub). `backend/backfill_scripts/**` deliberately excluded from the whole pipeline — one-time scripts, never auto-deployed.
  ...
  - **Two real bugs found on the first actual `production-backend` run (2026-07-17)**, both masked as a single opaque failure at first:
    ...
    2. Underneath that: `git pull` had been silently failing on every SSM-driven deploy (`fatal: detected dubious ownership in repository` — SSM runs as root, the checkout is owned by `ec2-user` from past manual `git pull` over SSH). The box's `docker-compose.yml` was still the pre-registry-image version as a result, so `docker compose pull` also failed (wrong image name, no `adarshmurali/` namespace). Fixed by adding an idempotent `git config --global --add safe.directory /home/ec2-user/FinSight-AI` as the first SSM command in both `cd.yml` jobs, rather than a one-off manual fix on the box — keeps the fix in version control and self-healing for the Flink EC2 (same ownership setup) and any future box rebuild.
  - **Next**: re-run `production-backend` end to end with both fixes live, then decide on approving `production-flink` (deliberately left un-approved so far, to verify the two Environments gate independently).
