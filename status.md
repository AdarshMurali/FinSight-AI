...
- 🔄 Task 6.3: Real CI/CD — in progress (2026-07-17). Plan: GitHub Actions for both CI (build/push Docker images to the user's Docker Hub) and CD (deploy via SSM, gated behind separate manual-approval GitHub Environments per EC2 — `production-backend`/`production-flink` — so a backend-only change can be approved without also redeploying Flink EC2, and vice versa). AWS auth via OIDC federation (no long-lived AWS keys stored in GitHub). `backend/backfill_scripts/**` deliberately excluded from the whole pipeline — one-time scripts, never auto-deployed.
  ...
  - **Known gap in today's idempotency fix**: `job_is_running()` only checks job *name*, not code version — editing `volatility_detector_job.py`/`news_sentiment_job.py` and redeploying while the old job is still healthy will silently skip resubmission, so the new code never loads until the job naturally stops. Proposed fix (not yet implemented): track a content hash per job file (e.g. `/home/ec2-user/.flink_job_versions/<job>.sha256`), and if a `RUNNING` job's name matches but the stored hash differs from the current file's hash, cancel and resubmit instead of skipping.
