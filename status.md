...
- ✅ Task 7.3: Volatility Detector Overhaul + RAG Cross-Ticker Fix + DB-Driven Coverage (2026-07-18). Four related fixes, full writeup in `plan.md` Task 7.3 — summarized here:
  ...
  - **Not yet deployed**: 4 commits on `adarsh` (`a353384`, `130d007`, `32296d4`, `5a07011`), CI will rebuild `backend`/`mcp` + `producer` images (not `flink` — its Dockerfile is untouched), both CD jobs pending manual approval. **Known gap**: the ticker-list commit only changed `config.py` (not `volatility_detector_job.py` itself) — `start_pipeline.sh`'s hash-based redeploy tracks job entrypoint files, not shared imports, so the running job may need a manual resubmit to pick up the dynamic fetch even after `deploy-flink` lands.
