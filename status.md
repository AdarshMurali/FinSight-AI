...
| 6 — Infrastructure | 🔄 In Progress | Backend cloud deploy ✅ (6.3 partial) · 6.2 Redis/Valkey caching ✅ · 6.4 JWT auth ✅ · 6.1 Dockerization ✅ (both EC2s, 2026-07-16) · real CI/CD still pending |
...
- 🔄 Task 6.3: Real CI/CD — in progress (2026-07-17). Plan: GitHub Actions for both CI (build/push Docker images to the user's Docker Hub) and CD (deploy via SSM, gated behind separate manual-approval GitHub Environments per EC2 — `production-backend`/`production-flink` — so a backend-only change can be approved without also redeploying Flink EC2, and vice versa). AWS auth via OIDC federation (no long-lived AWS keys stored in GitHub). `backend/backfill_scripts/**` deliberately excluded from the whole pipeline — one-time scripts, never auto-deployed.
  ...
  - **Verified the git-pull deploy path for non-containerized scheduled scripts (2026-07-17)**, and corrected a wrong assumption while doing it: `risk_job.py` and `price_update_job.py` actually run on the **Flink EC2** (`i-06df445415d082798`, via `mcpvenv`), not the backend EC2 — confirmed directly from both EventBridge Scheduler targets (`aws scheduler get-schedule --name finsight-daily-risk-job`/`finsight-daily-price-update`, both target `i-06df445415d082798`). Pushed a temporary marker log line into `risk_job.py`, deployed through `production-backend` first (wrong box, harmless), then through `production-flink` (right box), confirmed the file changed on the Flink EC2's checkout, then **actually executed `risk_job.py` live** (`mcpvenv/bin/python scripts/risk_job.py`) and saw the marker in real runtime output alongside genuine work (DB connect, 50 portfolios, live ChromaDB/OpenAI calls) — not just a file-presence check. Reverted the marker and redeployed the cleanup to both boxes, confirmed gone on both. `market_events_job.py` shares the identical deploy mechanism (same box, same `git pull`), so this result covers it too by the same reasoning, though not separately executed.
  - **Known gap, called out but not yet fixed**: ChromaDB has zero CI/CD coverage by original design (`aws/ec2-backend/docker-compose.yml` comment: "already Dockerized, already stable... left untouched in its own compose project") — contradicts the original "everything except one-time backfill" goal. Open decision, not yet resolved.
